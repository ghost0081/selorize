...
class SellScreenState extends State<SellScreen> {
  ...
  void _applyInitialSelection() {
    ...
    if (brandName != null && brandName.isNotEmpty) {
      ...
      if (_isCheckingBrandModels) {
        _brandModelEmptyTimer = Timer(const Duration(seconds: 8), () {
  ...
  Future<void> _loadSellData() async {
    setState(() {
    ...
    try {
      ...
      final results = await Future.wait([
        _repo.getData(tableName: 'brand'),
        _repo.getData(tableName: 'series'),
        _repo.getData(tableName: 'model'),
        _repo.getData(tableName: 'modelVariant'),
        _repo.getData(tableName: 'coupon', filter: {'status': '1'}),
        if (userId.isNotEmpty)
          _repo.getData(tableName: 'address', filter: {'userId': userId})
        else
          Future.value(<Map<String, dynamic>>[]),
        if (userId.isNotEmpty)
          _repo.getData(tableName: 'bankDetail', filter: {'userId': userId})
        else
          Future.value(<Map<String, dynamic>>[]),
      ]);
      ...
      setState(() {
        ...
        _apiCoupons = results[4].where(_isCouponActiveToday).toList();
        if (_apiBrands.isEmpty || _apiModels.isEmpty || _apiVariants.isEmpty) {
          _sellDataErrorMessage =
              'Could not load device data. Please check your network and try again.';
        }
        _savedAddresses = results[5];
        _savedBankDetails = results[6];
        if (userId.isEmpty) {
  ...
  void _selectBrand(String name) {
    ...
    _brandModelEmptyTimer = Timer(const Duration(seconds: 8), () {
      if (!mounted || _selectedBrand != canonicalName) return;
      setState(() => _isCheckingBrandModels = false);
    });
  }
  ...
  Future<void> _selectSeries(String? seriesId) async {
    ...
    setState(() {
      ...
      _modelListSearchQuery = '';
      _modelListSearchController.clear();
      _isLoadingSeriesModels = seriesId != null;
    });

    if (seriesId == null) return;

    try {
      final models = await _repo.getData(
        tableName: 'model',
      ...
      setState(() {
        _seriesFilteredModels = models;
      });
    } catch (e) {
      debugPrint('Series models error: $e');
      if (!mounted || _selectedSeriesId != seriesId) return;
      setState(() {
        _sellDataErrorMessage =
            'Could not load series models. Please check your network and try again.';
      });
    } finally {
      if (mounted && _selectedSeriesId == seriesId) {
        setState(() => _isLoadingSeriesModels = false);
      }
    }
  }
                      ...
                      return ListTile(
                        leading: Container(
                          ...
                          child: ClipRRect(
                            borderRadius: BorderRadius.circular(10),
                            child: () {
                              final imgUrl = _modelImageUrlByName(modelName);
                              if (imgUrl != null) {
  ...
  Widget _buildDeviceImage({
    ...
    return Image.network(
      ...
      width: width,
      fit: fit,
      errorBuilder: (context, error, stackTrace) => _buildFallbackDeviceImage(
        height: height,
        width: width,
        fit: fit,
        iconSize: iconSize,
      ...
      children: [
        ...
        Expanded(
          child: _isLoadingSeriesModels
              ...
              ? _buildApiDataMessage(
                  _sellDataErrorMessage ??
                      'No models are available for this brand right now.',
                  onRetry: _loadSellData,
                )
              : models.isEmpty
              ? const Center(
                          ...
                          child: LayoutBuilder(
                            builder: (context, constraints) {
                              return SingleChildScrollView(
                                key: PageStorageKey('choose_by_series_scroll_view_$_selectedBrand'),
  ...
  Widget _buildDeviceDetails() {
    ...
    if (_selectedStorage.isNotEmpty && !storageOptions.contains(_selectedStorage)) {
      _selectedStorage = '';
    }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 10),
                          const Text(
                            'Please select a storage option to calculate the value of your device.',
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              fontSize: 14,
                              color: Color(0xFF64748B),
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: ElevatedButton(
            onPressed: () {
              final title = _questionTitle(question).toLowerCase();
              final isHowOldQuestion = title.contains('how old');
              final isAnswered = isCheckbox
                  ? selectedSet.isNotEmpty
                  : (selectedOptionId != null && selectedOptionId.isNotEmpty);
              if (isHowOldQuestion && !isAnswered) {
                showDialog(
                  context: context,
                  builder: (context) => Dialog(
                    shape: RoundedRectangleBorder(
